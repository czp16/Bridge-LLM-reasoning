...
from sverl.utils.torch_functional import get_cosine_schedule_with_warmup
...
from sverl.utils.dataset import SFTDataset
from sverl.utils.fs import copy_to_local
from sverl.utils.tracking import Tracking
from sverl.utils.ulysses import get_ulysses_sequence_parallel_world_size, set_ulysses_sequence_parallel_group
from torch.distributed.device_mesh import DeviceMesh

import sverl.utils.hdfs_io as hdfs_io
from sverl.utils.debug import log_gpu_memory_usage
from peft import LoraConfig, TaskType, get_peft_model

from sverl.workers.sharding_manager import FSDPUlyssesShardingManager
...
class FSDPSFTTrainer(object):
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh):
        ...
        self.tokenizer = hf_tokenizer(model_path, trust_remote_code=self.config.model.trust_remote_code)
        ...
        def process_fn(example):
            return {
                "text": self.tokenizer.apply_chat_template(
                    example[config.data.text_key],
                    tokenize=False,
                ),
            }

        train_dataset = datasets.load_dataset("parquet", data_files={
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        ...
        loss_mask = batch['attention_mask'][:, :-1].reshape(-1).cuda()
