...
import sverl.utils.torch_functional as sverl_F
from omegaconf import DictConfig, open_dict
from sverl import DataProto
from sverl.single_controller.base import Worker
from sverl.single_controller.base.decorator import register, Dispatch
from sverl.utils import hf_tokenizer
from sverl.utils.debug import log_gpu_memory_usage
from sverl.utils.fs import copy_to_local
from sverl.utils.fsdp_utils import get_fsdp_wrap_policy, init_fn, get_init_weight_context_manager
from sverl.utils.fsdp_utils import offload_fsdp_optimizer, offload_fsdp_model_to_cpu, load_fsdp_optimizer, \
    load_fsdp_model_to_gpu
from sverl.utils.import_utils import import_external_libs
from sverl.utils.model import compute_position_id_with_mask
from sverl.utils.flops_counter import FlopsCounter
from sverl.utils.checkpoint_manager.fsdp_checkpoint_manager import FSDPCheckpointManager
from sverl.workers.sharding_manager.fsdp_ulysses import FSDPUlyssesShardingManager

from codetiming import Timer

logger = logging.getLogger(__file__)
logger.setLevel(os.getenv('sverl_PPO_LOGGING_LEVEL', 'WARN'))
...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
                               ...
                               use_liger=False,
                               role='actor'):
        from sverl.utils.model import print_model_size, update_model_config, get_generation_config
        from sverl.utils.torch_dtypes import PrecisionType
        ...
        if use_remove_padding:
            from sverl.models.registry import check_model_support_rmpad
            check_model_support_rmpad(actor_model_config.model_type)

        if use_remove_padding and self.ulysses_sequence_parallel_size > 1:
            from sverl.models.transformers.monkey_patch import apply_monkey_patch
        ...
        if role == 'actor':
            from sverl.utils.torch_functional import get_constant_schedule_with_warmup
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
            from sverl.workers.rollout import HFRollout
            from sverl.workers.sharding_manager import BaseShardingManager
            rollout = HFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
            rollout_sharding_manager = BaseShardingManager()
            # TODO: a sharding manager that do nothing?
        elif self.config.rollout.name == 'vllm':
            from sverl.workers.rollout.vllm_rollout import vLLMRollout
            from sverl.workers.sharding_manager import FSDPVLLMShardingManager
    ...
    def init_model(self):
        from sverl.workers.actor import DataParallelPPOActor
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        from sverl.utils.model import LambdaLayer, print_model_size, squeeze
        from sverl.utils.torch_dtypes import PrecisionType
    ...
    def init_model(self):
        ...
        from sverl.workers.critic import DataParallelPPOCritic
...
class RewardModelWorker(Worker):
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        from sverl.utils.ulysses import ulysses_pad_and_slice_inputs, gather_outpus_and_unpad
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            input_ids, attention_mask = sverl_F.tokenize_and_postprocess_data(
    ...
    def compute_rm_score(self, data: DataProto):
        import itertools
        from sverl.utils.seqlen_balancing import rearrange_micro_batches, get_reverse_idx
        data = data.to('cuda')
